...
export default function LoginModal({ onClose }: LoginModalProps) {
  ...
  const loginSchema = useMemo(() => z.object({
    email: z.string().email(dictionary.auth.login.errors.emailRequired),
    password: z.string().min(6, dictionary.auth.login.errors.passwordMin),
  }), [locale]);
  ...
  }, [touchedFields, formData, loginSchema]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(dictionary.auth.login.errors.loginFailed);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleWalletLogin = async () => {
    ...
    try {
      ...
      setError(dictionary.auth.login.errors.walletConnectionFailed);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleTelegramLogin = async () => {
    ...
    try {
      ...
      setError(dictionary.auth.login.errors.telegramLoginFailed);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const formFields = useMemo(() => [
    {
      id: 'email',
      label: dictionary.auth.login.emailAddress,
      type: 'email',
    ...
    },
    {
      ...
      type: 'password',
      placeholder: dictionary.auth.login.enterPassword,
      icon: Lock,
      showToggle: true,
    },
        ...
        >
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">
              {dictionary.auth.login.welcomeBack}
            </h2>
            <p className="text-zinc-400">{dictionary.auth.login.signInContinue}</p>
          </div>
        </div>
              ...
              >
                {isLoading ? (
                  <div className="flex items-center space-x-2">
                    ...
                    <span>{dictionary.auth.login.signingIn}</span>
                  </div>
                ) : (
                  <div className="flex items-center justify-center space-x-2">
                    <span>{dictionary.auth.login.signIn}</span>
                    <ArrowRight size={18} />
                  </div>
                )}
              </Button>
